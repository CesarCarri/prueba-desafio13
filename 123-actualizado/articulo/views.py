...
def listar_articulos_por_categoria(request, categoria_id):
    categoria = Category.objects.get(pk=categoria_id)
    articulos = Articles.objects.filter(categoria=categoria)
    return render(request, 'listar_articulos_por_categoria.html', {'categoria': categoria, 'articulos': articulos})

class listar_categorias(View):
    def get(self, request, *args, **kwargs):
        categorys = Category.objects.all()
        context = {
            'categorys': categorys
...
class listar_articulos_por_categorias(View):
    def get(self, request, *args, **kwargs):
        categorys = Category.objects.all()
        articles = Articles.objects.all()
        context = {
